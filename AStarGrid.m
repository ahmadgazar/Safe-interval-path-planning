...
% Evaluate Heuristic function, H, for each grid cell (estimated cost)
% Manhattan distance
H = abs(X - xd) + abs(Y - yd);  
H = H';
...
g = Inf(nrows,ncols);  %Cost to arrive to a cell
...
while true
    ...
    if ((current == dest_node) || isinf(min_f)) %goal arrived?
        break;
    end
    
    % Update input_map
    map(current) = 3; %mark as visited
    f(current) = Inf; % remove this node from further consideration (visited nodes can not be revisited)
    ...
    % entries in the map, f, g and parent arrays
        ...
        for nums = 1:4
               if adjacent(nums)~= inf  
                    if(map(adjacent(nums)) == 1 || map(adjacent(nums)) == 6)
